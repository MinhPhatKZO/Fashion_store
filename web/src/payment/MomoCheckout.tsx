...
const MomoCheckout: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      try {
        ...
        console.error(err);
        alert("Lỗi kết nối khi lấy đơn hàng");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. Cập nhật shippingAddress nếu người dùng nhập
      ...
      if (payData.payUrl) {
        ...
        alert(`Lỗi tạo giao dịch MoMo: ${payData.message || "Không xác định"}`);
        setIsProcessing(false);
      }
    } catch (err) {
      console.error(err);
      alert("Lỗi server khi kết nối cổng thanh toán!");
      setIsProcessing(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto pt-28 pb-16 px-4">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-black text-stone-900 uppercase tracking-tighter flex items-center gap-3">
          Thanh toán qua <span className="text-pink-600">MoMo</span>
        </h2>
      </div>
      ...
      <div className="grid md:grid-cols-12 gap-8">
          ...
          >
            ...
            <div className="space-y-4">
              <div className="relative group">
                <User className="absolute left-3 top-3 w-5 h-5 text-stone-400 group-focus-within:text-pink-600 transition-colors" />
                <input
                  type="text"
                  placeholder="Họ và tên người nhận"
                  value={fullname}
                  onChange={(e) => setFullname(e.target.value)}
                  className="w-full pl-10 pr-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:ring-2 focus:ring-pink-600 focus:bg-white transition-all outline-none"
                />
              </div>

              <div className="relative group">
                <Phone className="absolute left-3 top-3 w-5 h-5 text-stone-400 group-focus-within:text-pink-600 transition-colors" />
                <input
                  type="tel"
              ...
              <div className="relative group">
                <MapPin className="absolute left-3 top-3 w-5 h-5 text-stone-400 group-focus-within:text-pink-600 transition-colors" />
                <textarea
                  rows={3}
                  placeholder="Địa chỉ nhận hàng chi tiết"
                  value={address}
                  onChange={(e) => setAddress(e.target.value)}
                  className="w-full pl-10 pr-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:ring-2 focus:ring-pink-600 focus:bg-white transition-all outline-none resize-none"
                />
              </div>
            </div>
            
            <p className="text-xs text-stone-500 mt-4 italic">
              * Vui lòng điền đầy đủ thông tin để shipper có thể giao hàng chính xác nhất.
            </p>
          </motion.div>
        </div>
        ...
        <div className="md:col-span-5">
          ...
          >
            ...
            <div className="bg-white p-4 rounded-xl border border-pink-100 mb-6 flex items-start gap-3">
              <div className="w-10 h-10 flex-shrink-0 border border-stone-100 rounded-lg p-1 bg-white">
                 <img src="https://cdn.haitrieu.com/wp-content/uploads/2022/10/Logo-MoMo-Square.png" alt="MoMo" className="w-full h-full object-contain rounded" />
              </div>
              <div>
                <p className="text-sm font-bold text-stone-900">Ví điện tử MoMo</p>
                <p className="text-xs text-stone-500 mt-1">
                  Bạn sẽ được chuyển hướng sang ứng dụng/website MoMo để hoàn tất thanh toán an toàn.
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full py-4 px-6 rounded-xl font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-lg
                ${isProcessing 
                  ...
                  : "bg-pink-600 text-white hover:bg-pink-700 hover:shadow-pink-200 active:scale-[0.98]"
                }`}
            >
              {isProcessing ? (
                <>
            ...
            <div className="mt-4 flex items-center justify-center gap-2 text-xs text-stone-400">
              <ShieldCheck className="w-3 h-3" /> Bảo mật thanh toán bởi MoMo
            </div>
